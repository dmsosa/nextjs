...
import { rechnungBearbeiten, TActionState } from '@/app/lib/actions';
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          <div id="customer-error" aria-live="polite" aria-atomic="true">
            {state.errors?.customerId && 
              state.errors?.customerId.map((errorMessage) => (
        ...
        <div className="mb-4">
          ...
          <div className="relative mt-2 rounded-md">
            ...
            {state.errors?.amount && 
              state.errors?.amount.map((errorMessage) => (
                <p className="mt-2 text-sm text-red-500" key={errorMessage}>{errorMessage}</p>
              ))
            }
          </div>
          ...
          <div className="rounded-md border border-gray-200 bg-white px-[14px] py-3">
            <div className="flex gap-4">
              ...
              <div id="status-error" aria-live="polite" aria-atomic="true">
                {state.errors?.status && 
                  state.errors?.status.map((errorMessage) => (
                    <p className="mt-2 text-sm text-red-500" key={errorMessage}>{errorMessage}</p>
                  ))
                }
              </div>
